...
- [CI builds](https://ci.infra.status.im/job/website/job/guide.nimbus.team/) `master` and pushes to `deploy-master` branch, which is hosted at <https://guide.nimbus.team//>.
- [CI builds](https://ci.infra.status.im/job/website/job/dev-guide.nimbus.tean/) `develop` and pushes to `deploy-develop` branch, which is hosted at <https://dev-guide.nimbus.team//>.
